...
def strip_transport_marker(value: str, marker: str) -> str:
...
def decode_partner_blob(value: str) -> bytes:
...
def resolve_asset_path(root: str, rel: str) -> str:
    return os.path.join(root, strip_transport_marker(rel or "index.html", "asset:"))


def resolve_asset_name(root: str, rel: str) -> str:
...
def parameterized_filter(fields: dict[str, str]) -> tuple[str, tuple[Any, ...]]:
    q = fields.get("q", "")
