---
description: "Use when setting up Endor Labs Agent Kit for Kiro, checking readiness, verifying Endor auth, choosing namespaces, or diagnosing missing endorctl, gh, Endor MCP, or workflow prerequisites."
...
<!-- endor_agent_kit_managed=true agent_id=endor-agent-kit-setup agent_name=endor-agent-kit-setup-agent host=kiro -->

# Endor Agent Kit Setup Agent For Kiro

Generated for the Endor Labs Agent Kit Kiro plugin agent package.

## Bundled Kiro Workflows
...
## Kiro Plugin Install Notes

Install or update this package through Kiro's plugin-loading mechanism only after user approval. The generated Kiro plugin uses repository-root `.kiro-plugin/` metadata, root `agents/`, root `skills/`, `hooks/`, and `assets/logo.png`.

This Kiro plugin is separate from the Gemini CLI extension under `plugins/gemini/endor-labs-agent-kit/`. Do not use Kiro installation steps to install Gemini CLI files, and do not use Gemini extension files as Kiro package metadata.
...
- Keep Kiro plugin installs explicit. Do not install, link, update, or uninstall packages without user approval.
...
- Tell the user to reload or restart Kiro after installing or updating the plugin if newly installed agents or skills are not visible.
