...
<!-- endor_agent_kit_managed=true agent_id=cicd-posture agent_name=endor-cicd-posture-agent host=kiro -->

# CI/CD And Supply Chain Posture

Generated from Endor Agent Kit recipe `cicd-posture` v0.1.0 for the Endor Labs Agent Kit Kiro plugin agent.
...
- Treat the loaded CI/CD Posture artifact as authoritative for this run. Do not
  ...
  `.claude`, `.codex`, `.kiro`, or `.gemini` directories for a second copy of
